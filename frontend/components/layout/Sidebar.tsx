  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center px-4 pt-4 pb-3 shrink-0">
        <div
          className={`
            overflow-hidden rounded-2xl shrink-0
            ${collapsed ? 'w-9 h-9' : 'w-[70%] aspect-square'}
          `}
        >
          <img
            src="/logo.png"
        ...
        {!collapsed && (
          <div className="text-center mt-1">
            <p className="text-slate-800 text-lg leading-tight tracking-wide font-semibold">
              Intranet Avalanz
            </p>
            <p className="text-slate-400 text-xs mt-0.5 tracking-widest uppercase">
              v1.0.0
            </p>
          </div>
        )}
        ...
        {isAdmin() && (
          <div className="px-2">
            ...
            <nav className="space-y-0.5">
              {adminItems.map((item) => {
                if (!isSuperAdmin() && ['/admin/groups', '/admin/modules', '/admin/roles', '/admin/permissions'].includes(item.href)) {
                  return null
                }
                return (
                  <NavLink
    ...
    className={`
      ...
      ${item.isModule
        ? active
          ...
          : 'text-slate-800 hover:bg-[#1a4fa0] hover:text-white font-semibold border border-slate-200'
        : active

...
import {
  ...
  AlertCircle, Loader2, Download, Paperclip, Upload, X, FileCheck, Search, Users
} from 'lucide-react'
...
const Step3Anexos = ({ attachmentDefs, uploadedFiles, onUpload, onRemove }: {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-slate-500 mb-4">Adjunta los documentos antes de enviar el contrato.</p>
      <div className="space-y-3">
        {attachmentDefs.map(def => {
          ...
          return (
            <div key={def.id} className={`border-2 rounded-xl p-3.5 transition-all ${uploaded ? 'border-green-300 bg-green-50' : def.is_required ? 'border-slate-200 bg-white' : 'border-dashed border-slate-200 bg-white'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${uploaded ? 'bg-green-100' : 'bg-slate-100'}`}>
                  {uploaded ? <FileCheck size={16} className="text-green-600" /> : <Paperclip size={16} className="text-slate-400" />}
                </div>
                <div className="min-w-0 flex-1">
...
const Step5 = ({ templateName, isSubmitting, onConfirm, error, attachmentCount }: {
  templateName: string; isSubmitting: boolean; onConfirm: () => void; error?: string; attachmentCount: number
}) => (
  <div className="text-center py-4">
    ...
    <h2 className="text-sm font-semibold text-slate-900 mb-1.5">¿Listo para enviar?</h2>
    <p className="text-xs text-slate-500 mb-2 max-w-sm mx-auto">Al confirmar, el contrato <strong>{templateName}</strong> será enviado al área legal para revisión.</p>
    {attachmentCount > 0 && <div className="flex items-center justify-center gap-1.5 text-xs text-green-600 mb-2"><FileCheck size={13} /><span>{attachmentCount} documento{attachmentCount > 1 ? 's' : ''} adjunto{attachmentCount > 1 ? 's' : ''}</span></div>}
    <p className="text-[11px] text-slate-400 mb-5 max-w-sm mx-auto">El SLA comenzará a contar desde este momento.</p>
    ...
    <button onClick={onConfirm} disabled={isSubmitting} className="flex items-center gap-2 bg-[#1a4fa0] text-white text-sm font-medium px-8 py-2.5 rounded-xl hover:bg-blue-700 disabled:opacity-50 transition mx-auto">
      {isSubmitting ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Enviando...</> : <><Send size={15} />Enviar al área legal</>}
    </button>
  </div>
)
...
export default function NuevoContratoPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        if (pdfUpload.data?.data?.object_key) await registerAttachment(pdfUpload.data.data, undefined, `Contrato generado — ${folio}`)
      ...
      for (const uploaded of uploadedFiles) {
        try {
          ...
          if (aUpload.data?.data?.object_key) await registerAttachment(aUpload.data.data, uploaded.defId)
  ...
  return (
    <PageWrapper title="Crear contrato" description="Completa el formulario y envía tu solicitud al área legal"
      actions={<button onClick={handleBack} className="flex items-center gap-1.5 text-sm text-slate-600 border border-slate-300 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition"><ArrowLeft size={14} />{step === 1 ? 'Cancelar' : 'Atrás'}</button>}>

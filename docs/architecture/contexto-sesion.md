...
**Rama activa:** `develop`
**Próxima tarea:** Iniciar el primer módulo operativo (Bóveda DYCE o Legal)
...
### Monitoreo
- Prometheus + Grafana con métricas de los 6 servicios FastAPI
...
### Servicios corriendo
- 19 contenedores Docker (todos los microservicios + exporters + mailpit + cron)
- Frontend Next.js con PM2 (proceso: `intranet-frontend`, puerto 3000)
- Scaffold-server con PM2 (proceso: `scaffold-server`, puerto 3002)
- PostgreSQL con 3 BDs: avalanz_auth, avalanz_admin, avalanz_notify
- Seeder ejecutado: 75 empresas, 2 grupos, super admin `admin@avalanz.com / Admin@2026!`

### Variables de entorno críticas en VM
- `SCAFFOLD_SERVER_URL=http://10.12.0.250:3002` en `backend/admin-service/.env`
- `NEXT_PUBLIC_SCAFFOLD_URL=http://10.12.0.51:3002` en `frontend/.env.local`
- `NEXT_PUBLIC_API_URL=http://10.12.0.51` en `frontend/.env.local`
- SMTP: Office 365 via soporte@avalanz.com, remitente no-reply@avalanz.com
...
El `server_name` incluye `_` como catch-all para funcionar sin header Host:
```nginx
...
- Contraseñas y JWT keys de producción
- Firewall Fortinet configurado
- Merge a main (cuando Edgar y Andrés aprueben el cascarón)
